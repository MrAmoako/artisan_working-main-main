...
const Client = () => {
  ...
  const renderUsersByJob = (job) => {
    const filteredUsers = users.filter(user => user.job && user.job.toLowerCase() === job);
    return (
      <div className="p-4">
        ...
        {filteredUsers.map((user, index) => (
          <div key={index} className="p-2 cursor-pointer border-b flex items-center" onClick={() => setSelectedUser(user)}>
            {user.imageUrl ? (
              <img src={user.imageUrl} className='rounded-full h-11 w-11 mr-4' alt='Profile Picture' />
            ) : (
              <img src={background} className='rounded-full h-11 w-11 mr-4' alt='Default Profile Picture' />
            )}
            <div>
  ...
  const renderOthers = () => {
    const filteredUsers = users.filter(user => !['plumber', 'carpenter', 'mason'].includes(user.job && user.job.toLowerCase()));
    return (
      <div className="p-4">
